...
import UserService.Types      (AppM, Role (..), UpdateUser, User,
                               UserClaims (..), UserMsAppContext, UserSearch)
...
data UserApiRoutes mode = UserApiRoutes
  { userRoutes :: mode :- Auth '[JWT] (UserClaims 'NormalUser) :> NamedRoutes UserRoutes
  , adminRoutes :: mode :- Auth '[JWT] (UserClaims 'Admin) :> NamedRoutes AdminRoutes
  }
  deriving (Generic)
...
userServer = BaseUserApiRoutes UserApiRoutes
  { adminRoutes = \case
      Authenticated usr -> AdminRoutes
        { searchUsersRoute = withHashable . searchUsers
  ...
  , userRoutes = \case
      Authenticated usr -> UserRoutes
        { saveUserRoute = withHashable . saveUser
        , userCountRoute = totalUsers
        }
